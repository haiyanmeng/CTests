...
int main(int argc, char *argv[]) {
	...
	case 0: // child
		...
		if(dup2(out[1], STDIN_FILENO)) {
			fprintf(stderr, "`dup2(out[1], STDIN_FILENO)` failed: %s\n", strerror(errno));
			exit(EXIT_FAILURE);
		}
		if(dup2(err[1], STDIN_FILENO)) {
			fprintf(stderr, "`dup2(err[1], STDIN_FILENO)` failed: %s\n", strerror(errno));
			exit(EXIT_FAILURE);
		}
		...
		execlp("sh", "sh", "-c", "wc /home/hmeng/1", NULL);
	...
	while(1) {
		...
		int *status;
		ssize_t r;
		int output[1048576], error[1048576];
		size_t len_out, len_err;

		if(len == 0 && in[1] > 0) close(in[1]);
		...
		if(r == -1) {
			...
			fprintf(stdout, "the output of child process:\n%s\n\n", output);
		}

		r = read(err[0], error, len_err);
		if(r == -1) {
			if(errno != EAGAIN && errno != EINTR) {
			...
			fprintf(stderr, "the error of child process:\n%s\n\n", error);
		}
	}
